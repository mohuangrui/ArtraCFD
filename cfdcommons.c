 ...
 ****************************************************************************/
int ConvertConservativeToPrimitive(Field *field, const Space *space, const Flow *flow)
{
    /*
    ...
    for (k = 0; k < space->kMax; ++k) {
        for (j = 0; j < space->jMax; ++j) {
            for (i = 0; i < space->iMax; ++i) {
                ...
                p[idx] = (flow->gamma - 1) * (rho_eT[idx] - 0.5 * rho[idx] * (u[idx] * u[idx] + v[idx] * v[idx] + w[idx] * w[idx]));
